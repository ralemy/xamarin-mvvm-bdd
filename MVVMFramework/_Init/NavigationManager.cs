...
namespace MVVMFramework
{
    public class NavigationManager : INavigationManager, INavigationService, INotifyPropertyChanged
    {
        ...
        public void NavigateTo(string pageKey, object parameter)
        {
            lock (_pages)
            {
                if (pageKey.StartsWith("__", StringComparison.Ordinal))
                {
                    Register(pageKey, parameter.GetType());
                    _page.PushAsync(parameter as Page);
                }
                else if (!_pages.ContainsKey(pageKey))
